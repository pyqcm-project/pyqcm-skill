...
before writing code — the same concept (e.g. "an operator") exists at multiple layers with
different responsibilities, and touching the wrong one either won't compile or won't be reachable
from Python.

## Layers, outside-in

1. **`pyqcm/pyqcm/*.py`** — pure Python. This is where the user-facing API lives
   (`__init__.py`, ~3300 lines) plus higher-level pure-Python logic that doesn't need C++ performance:
   `cdmft.py` (CDMFT self-consistency driver), `vca.py` (VCA driver), `_loop.py` (parameter sweeps),
   ...
   themselves, it likely belongs here and can be developed/tested without touching CMake at all.
...
3. **`pyqcm/src_qcm/`** — the lattice/CPT-VCA-CDMFT engine: `lattice_model`/`lattice_model_instance`
   ...
   computed — the natural landing spot if/when Liouvillian interpolation (see `references/physics.md`,
   "Periodization / interpolation schemes") gets implemented as a new scheme, though as of this
   writing that work hasn't started — confirm current status before assuming otherwise.
...
5. **`pyqcm/src_util/`** — shared numerical utilities not specific to ED or lattice logic: dense/sparse
   matrix wrappers (`matrix.cpp`), BLAS/LAPACK shims (`lapack-blas.h`, `cblas.h`), HDF5 I/O
...
- Rebuild after any C++ change: see `references/build.md` (editable installs still trigger a full
  CMake reconfigure/rebuild — there's no standalone incremental-only path via `pip install -e .`).
- Run `pyqcm/tests/test_all.py` after non-trivial changes to `src_ed`/`src_qcm` — a change meant to
  be purely additive can silently perturb existing solvers (e.g. shared Hamiltonian-construction code
  paths).
...
- `pyqcm/bench/` (ED, integrals, VCA) holds performance benchmarks — check these if a change could
  plausibly affect performance-sensitive code paths (Hamiltonian construction/diagonalization,
  integration routines).

## Style

`src_ed/.clang-format` defines the C++ formatting convention for that directory — run it (or match
its style by eye) rather than introducing a different style within the same files.
