...
Every pyqcm calculation follows the same shape, regardless of whether the underlying method ends up
being plain CPT, VCA, or CDMFT. `pyqcm/notebooks/intro1.py` is the canonical minimal example; read it
before writing a new script, then adapt from the closest matching notebook in `pyqcm/notebooks/`
rather than starting from a blank page — there's already a worked example for AF order, CDW,
superconductivity, Rashba coupling, slab/heterostructure geometries, and graphene.

`pyqcm/tests/test_files/` is a second, larger source of worked examples — it's the actual test suite
(auto-discovered and run by `test_all.py`), so every file there is a real, currently-passing script,
not illustrative pseudocode. It covers things `notebooks/` doesn't, e.g. subbath CDMFT
...
periodization (`test_periodize.py`). When a notebook doesn't cover the feature in question, check here
before assuming it isn't demonstrated anywhere in the codebase.
...
2. **Cluster** — `pyqcm.cluster(cluster_model, positions)` places a physical instance of that model
   ...
   `pyqcm/docs/source/cdmft.rst` ("Subbaths") for the mechanics (self-energies/hybridizations from
   each system are averaged by default when building the host).
...
4. **Operators** — declare the Hamiltonian terms on the model: `interaction_operator` (Hubbard U),
   `hopping_operator` (one-body/hopping), plus anomalous/density-wave/Hund/Heisenberg operators for
   more exotic order. See `pyqcm/docs/source/models.rst` for the full operator taxonomy and the sign
   /normalization conventions — get this wrong and the physics is subtly wrong without erroring.
...
7. **Instance** — `pyqcm.model_instance(model)` builds a concrete solvable instance from the current
   parameter values. For VCA/CDMFT you don't usually call this directly — `pyqcm.vca` /
...
1. **Is the extension actually built?** See `references/build.md` — a missing/stale build produces
...
4. **Does the mixing state match what you intended?** Adding an anomalous or spin-flip operator
   changes the Green function's mixing state (normal → spin-asymmetric → spin-flip → Nambu — see
