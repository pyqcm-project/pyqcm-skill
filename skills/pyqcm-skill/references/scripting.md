...
CPT, VCA, or CDMFT. `$PYQCM_ROOT/notebooks/intro1.py` is the canonical minimal example; read it before
writing a new script, then adapt from the closest matching notebook in `$PYQCM_ROOT/notebooks/` rather than
...
`$PYQCM_ROOT/tests/test_files/` is a second, larger source of worked examples. It is the actual test suite
...
2. **Cluster**: `pyqcm.cluster(cluster_model, positions)` places a physical instance of that model
   ...
   and `$PYQCM_ROOT/docs/source/cdmft.rst` ("Subbaths") for the mechanics (self-energies and
...
4. **Operators**: declare the Hamiltonian terms on the model: `interaction_operator` (Hubbard U),
   ...
   operators for more exotic order. See `$PYQCM_ROOT/docs/source/models.rst` for the full operator taxonomy
...
| Topic | Doc source |
|---|---|
| Cluster model definition, bath sites | `$PYQCM_ROOT/docs/source/defining_cluster_models.rst` |
| Lattice model definition, superlattice vectors | `$PYQCM_ROOT/docs/source/defining_models.rst` |
| Operator types (hopping, anomalous, density wave, Hund, Heisenberg), mixing states, orbital/sector index conventions | `$PYQCM_ROOT/docs/source/models.rst` |
| Parameter sets, `set_parameters` syntax | `$PYQCM_ROOT/docs/source/parameters.rst` |
| CDMFT self-consistency loop, incl. subbath (multiple bath systems per cluster) | `$PYQCM_ROOT/docs/source/cdmft.rst` |
| VCA (Potthoff functional minimization) | `$PYQCM_ROOT/docs/source/vca.rst` |
| Spectral functions, Green's functions, periodization | `$PYQCM_ROOT/docs/source/spectral.rst`, `$PYQCM_ROOT/pyqcm/green_structure.py` (docstrings; the rendered `green_structure` doc page is autodoc-generated from this module, not a standalone `.rst`) |
| Hartree/mean-field treatment of extended interactions | `$PYQCM_ROOT/docs/source/hartree.rst` |
| Sweeping a parameter (phase diagrams) | `$PYQCM_ROOT/docs/source/loop.rst` |
| Plotting/drawing clusters and operators | `$PYQCM_ROOT/docs/source/draw.rst` |
| Slab / heterostructure / hybrid lattice geometries | `$PYQCM_ROOT/docs/source/lattice_hybrid.rst` |
...
5. **Compare against a close notebook example** rather than the abstract docs. If a similar model
   already works in `$PYQCM_ROOT/notebooks/`, diff against it.
6. **Check `references/practice.md`** for anything already known about this exact failure mode.
...
`$PYQCM_ROOT/tests/test_all.py` (outputs land in `tests/test_outputs/`) rather than assuming a local fix is
isolated.
