...
>
> Some of what follows reflects choices made by the code owner (David Sénéchal) and contributers that could change in a future release. Those are flagged where they occur, so double-check rather than trusting them blindly.
...
>
> - $A_1\longrightarrow\;$R0, $A_1$ is even under $m_y$ and $m_x$: 00 = 0.
> - $A_2\longrightarrow\;$R3, $A_2$ is odd under both $m_y$ and $m_x$: 11 = 3.
> - $B_1\longrightarrow\;$R1, $B_1$ is odd under $m_x$: 01 = 1.
> - $B_2\longrightarrow\;$R2, $B_2$ is odd under $m_y$: 10 = 2.
...
- `pyqcm.set_global_parameter('temperature', 0.01)`:
    ...
    - **When**: Smoothing the sectors transitions when sweeping over a lattice parameter in CDMFT, i.e. studying one-site density as a function of the chemical potential (fig. 14 in the pyqcm paper, SciPost Phys. Codebases 23).
